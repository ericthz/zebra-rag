...
import { addColorAlpha, getColorPalette, getPaletteColorByNumber, getRgb } from '@zebra/color';
 ...
 * @param [recommended=false] Use recommended color. Default is `false`
 */
export function getNaiveTheme(colors: App.Theme.ThemeColor, recommended = false, darkMode = false) {
  const { primary: colorLoading } = colors;

  // 所有 Naive 组件背景统一为左侧菜单栏（container）的背景色，亮色白、暗色 #1c1c1c
  const containerColor = darkMode ? '#1c1c1c' : '#ffffff';
