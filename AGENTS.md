...
Zebra RAG is an enterprise-grade AI knowledge base management system built with RAG (Retrieval Augmented Generation) technology. It provides intelligent document processing, indexing, and retrieval capabilities with multi-tenant support.
...
```
cmd/server/          - API 网关入口（main.go，依赖装配 + 路由注册）
cmd/worker/          - 文档处理 Worker（独立进程运行 Kafka 消费者）
cmd/eval/            - RAG 离线评测运行器
internal/
  ├── bootstrap/     - 组合根：基础设施、仓储、服务、流水线的统一装配
  ├── config/        - Configuration management (Viper)
  ├── module/        - 领域模块（路由自注册）：auth/user/upload/document/search/conversation/chat/vision/admin
...
internal/infra/      - 外部集成（位于 internal，避免泄漏到公共包）
  ...
  ├── rerank/        - 重排服务客户端
  ├── sparse/        - 稀疏向量（BM25 类算法）客户端
  ├── storage/       - MinIO object storage client
  ├── tika/          - Apache Tika document parser client
  └── vision/        - 多模态图片理解客户端
pkg/                 - 可复用的公共工具包（log/token/tasks/metrics 等）
```

**Key Design Patterns:**
- **Dependency Injection**: `internal/bootstrap` 作为组合根统一装配依赖；领域模块通过 `RegisterRoutes` 自注册路由
- **Module 化**: handler 与路由按领域聚合在 `internal/module/*`
