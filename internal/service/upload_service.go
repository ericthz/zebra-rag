...
func (s *uploadService) UploadChunk(ctx context.Context, fileMD5, fileName string, totalSize int64, chunkIndex int, file multipart.File, userID uint, orgTag string, isPublic bool) ([]int, int, error) {
	...
	// 3. 将分片上传到 MinIO（先缓冲到内存计算分片 MD5，供 chunk_info 完整性校验）
	objectName := fmt.Sprintf("chunks/%s/%d", fileMD5, chunkIndex) // 与 Java 一致的路径
	chunkBytes, err := io.ReadAll(file)
	if err != nil {
		log.Errorf("[UploadChunk] 读取分片内容失败, chunk=%d, err=%v", chunkIndex, err)
		return nil, 0, err
	}
	chunkSum := fmt.Sprintf("%x", md5.Sum(chunkBytes))
	...
	chunkRecord := &model.ChunkInfo{
		...
		ChunkMD5:    chunkSum,   // 计算分片 MD5，替代原先的空串
...
func (s *uploadService) MergeChunks(ctx context.Context, fileMD5, fileName string, userID uint) (string, error) {
	...
	// 2. 根据分片数量选择合并策略
	destObjectName := fmt.Sprintf("merged/%s", fileMD5) // 以 fileMD5 为对象 key，避免跨用户同名覆盖
	...
	// 4. 触发 Kafka 消息
	objectURL, _ := storage.GetPresignedURL(s.minioCfg.BucketName, destObjectName, time.Hour)
