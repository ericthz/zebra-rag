...
type UploadRepository interface {
	...
	CountAccessibleFiles(userID uint, orgTags []string) (int64, error)
	UpdateVectorizedStatus(fileMD5 string, userID uint, status string) error
	UpdateVectorized(fileMD5 string, userID uint, status, errorMsg string, estimatedTokens, estimatedChunks, actualTokens, actualChunks int) error
...
// GetChunkInfoRecords 获取指定文件已上传的所有分块信息（来自数据库，合分片时使用）。
...
// CountFilesByUserID 统计用户已完成（status=1）上传的文档数量。
func (r *uploadRepository) CountFilesByUserID(userID uint) (int64, error) {
	var count int64
	err := r.db.Model(&model.FileUpload{}).Where("user_id = ? AND status = ?", userID, 1).Count(&count).Error
	return count, err
}

// CountAccessibleFiles 统计用户可访问的文档数量（本人 + 全局公开 + 所属组织公开）。
func (r *uploadRepository) CountAccessibleFiles(userID uint, orgTags []string) (int64, error) {
	var count int64
	err := r.db.Model(&model.FileUpload{}).
		Where("status = ?", 1).
		Where(r.db.Where("user_id = ?", userID).
			Or("is_public = ?", true).
			Or("org_tag IN ? AND is_public = ?", orgTags, true)).
...
// UpdateVectorizedStatus 更新文件的向量化处理状态（如标记为 processing）。
func (r *uploadRepository) UpdateVectorizedStatus(fileMD5 string, userID uint, status string) error {
	return r.db.Model(&model.FileUpload{}).
		Where("file_md5 = ? AND user_id = ?", fileMD5, userID).
		Update("vectorized_status", status).Error
}

// UpdateVectorized 更新文件的向量化结果：状态、失败原因、预估与实际 token/切片统计。
func (r *uploadRepository) UpdateVectorized(fileMD5 string, userID uint, status, errorMsg string, estimatedTokens, estimatedChunks, actualTokens, actualChunks int) error {
	return r.db.Model(&model.FileUpload{}).
		Where("file_md5 = ? AND user_id = ?", fileMD5, userID).
		Updates(map[string]interface{}{
			"vectorized_status": status,
			"vectorize_error":   errorMsg,
