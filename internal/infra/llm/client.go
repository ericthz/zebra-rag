...
type Client interface {
	// StreamChatMessages 以 role-based 消息与可选生成参数调用会话接口，并将流式分块写入 writer。
	...
	// Complete 非流式完成调用，用于查询改写等单轮文本生成。
...
func NewClient(cfg config.LLMConfig) Client {
	// 流式场景不设整体超时（长回答可能持续数分钟），但限制建连与响应头等待时间，
	// 避免模型服务无响应时请求永久挂起。
...
// Complete 非流式完成调用：返回完整文本。
func (c *deepseekClient) Complete(ctx context.Context, system, user string, maxTokens int) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", fmt.Errorf("completion api returned %s: %s", resp.Status, string(body))
	}
	...
	if len(cr.Choices) == 0 {
		return "", fmt.Errorf("completion api returned empty choices")
	}
