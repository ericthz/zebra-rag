...
type SearchResponseDTO struct {
	...
	ParentText  string  `json:"parentText"` // 父块内容，用于生成上下文
...
type EsDocument struct {
	...
	ParentText   string             `json:"parent_text"`             // 父块内容（不参与索引，仅存储）
	Vector       []float32          `json:"vector"`                  // 文本内容的稠密向量表示
	SparseVector map[string]float32 `json:"sparse_vector,omitempty"` // 稀疏向量（BGE-M3 sparse），可空
